...
In this game, the player controls a "character" that jumps from circle to circle.
...
{{% notice note %}}
We'll be using GLES 3 to start. It's not yet clear what if any impact this will
have. Once we get to the mobile testing phase, we'll see if a switch to GLES 2
is warranted.
{{% /notice %}}
...
To keep things organized, we're going to make a folder to hold the game objects
(`objects`) and one for UI (`gui`). The game assets (images, audio, etc.) will
go in an `assets` folder. You can download the starting assets here:
...
For movement and collision, we're going to use a `Area2D`. To be fair,
we could use `KinematicBody2D` here too, and it would work just as well. However,
we don't really need collision in this game, we just need to know when the
jumper contacts a circle. Let's add the following nodes:
...
Save the scene in `res://objects/` and drag the circle image (`res://assets/images/jumper.png`)
into the Sprite's _Texture_. Note that all the game images are flat white. This
will make it easier for us to dynamically color them later.

Since the art is drawn pointing upwards, set the `Sprite`'s _Rotation_ property
to `90`.
...
Next we'll detect the screen touch and, if we're on a circle, call our jump
method:
...
We'll detect hitting a circle with the `area_entered` signal, so connect it.
If we hit a circle, we'll stop moving forward.
...
If we are captured by a circle, we want to rotate around it. We'll add a
pivot on the circle, and match its transform so our orientation will always be
facing outwards. Otherwise we move forward in a straight line.
...
color. Select the `Sprite` and then in the _Material_ property add a new
...
script. Then it changes all the visible pixels of the texture into that color,
preserving the alpha (transparency) value.
...
The second game object is the circle, which will be instanced many times as
the game progresses. Eventually, we'll add a variety of behaviors such as
moving, shrinking, etc., but for this first iteration, we just want it to
capture the player.
...
The "Pivot" node is how we'll make the player orbit the circle. The "OrbitPosition"
will be offset by whatever the size of the circle is, and the player will follow
it.

Use `res://assets/images/circle1_n.png` as the `Sprite`'s texture. While we're
here, add a `ShaderMaterial` and choose "Load" to use the saved `color.shader`
we made earlier.

Add a circle shape to the `CollisionShape2D` and attach a script to the root
node.
...
In the `init()` function, we're setting up the size of the circle, based on
the given `radius`. We need to size the collision shape as well as scaling
the texture to match.

Try running the scene with different values of `radius` to test. (Later we'll
stop calling `init()` in `_ready()`).

## Main Scene

Now we can test out the interaction.

Create a "Main" scene using a `Node2D` and instance both the Jumper and the
Circle in it. Arrange them so the jumper will hit the Circle (Jumper's default
velocity is `(100, 0)`).

Try running. You should see the jumper get captured by the circle and start
orbiting it. Clicking the mouse should then send the jumper flying off in
whatever direction it's pointing.
